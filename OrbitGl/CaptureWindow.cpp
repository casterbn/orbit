...
void CaptureWindow::MiddleUp(int a_X, int a_Y) {
  ...
  m_SelectStop = Vec2(worldx, worldy);
  // time_graph_.SelectEvents( m_SelectStart[0], m_SelectStop[0], -1 );
...
void CaptureWindow::RenderUI() {
  ...
  if (m_DrawStats) {
    ...
    m_StatsWindow.AddLine(VAR_TO_ANSI(time_graph_.GetNumDrawnTextBoxes()));
    m_StatsWindow.AddLine(VAR_TO_ANSI(time_graph_.GetNumTimers()));
    m_StatsWindow.AddLine(VAR_TO_ANSI(time_graph_.GetThreadTotalHeight()));
